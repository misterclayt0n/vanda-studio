<script lang="ts">
    import { Button, Badge, Input, Label, Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from "$lib/components/ui";
    ...
    function openEditModal() {
        ...
        editInstagramUrl = project.instagramUrl;
        showEditModal = true;
    }

    // Handle update
    async function handleUpdate() {
        if (!editName.trim() || !editInstagramUrl.trim()) return;
            ...
            <div class="shrink-0 border-b border-border bg-muted/30 px-6 py-4">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <Button variant="outline" onclick={() => goto(`/projects/${projectId}/settings`)}>
                            <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
{#if showEditModal}
    ...
    <div class="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md border border-border bg-background shadow-xl">
        <div class="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 class="text-lg font-semibold">Editar Projeto</h2>
            <button
                type="button"
                aria-label="Fechar"
        ...
        <div class="p-6 space-y-4">
            <div class="space-y-2">
                <Label for="edit-name" class="text-sm font-medium">Nome do Projeto</Label>
                <Input
                    id="edit-name"
                    bind:value={editName}
            ...
            <div class="space-y-2">
                <Label for="edit-url" class="text-sm font-medium">URL do Instagram</Label>
                <Input
                    id="edit-url"
        ...
        <div class="flex items-center justify-end gap-3 border-t border-border px-6 py-4">
            <Button variant="outline" onclick={() => showEditModal = false} disabled={isUpdating}>
                Cancelar
            </Button>
            <Button onclick={handleUpdate} disabled={!editName.trim() || !editInstagramUrl.trim() || isUpdating}>
                {#if isUpdating}
