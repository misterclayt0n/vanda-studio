...
│ │12/01/26 │ │12/01/26 │ │11/01/26 │ │11/01/26 │        │
│ └─────────┘ └─────────┘ └─────────┘ └─────────┘        │
│                                                         │
│ ┌─────────┐ ┌─────────┐ ...                            │
└─────────────────────────────────────────────────────────┘
...
- Click card → opens detail modal or navigates to post page
...
3. **Gallery test flow**:
   - Generate a post → appears in gallery
   - Click card → see details
   - Delete → goes to trash
   - Restore → back in gallery
...
- **"Start Editing" button** → creates conversation, navigates to `/posts/edit/[conversationId]`

### 3. Conversation Page
- **Header**: Back button (← Gallery), title (auto-generated from first prompt), turn count
...
│  └──────────────┘                                    │
│   Model Name                 ┌────────────────────┐  │
│   1024 x 1024                │ Your instructions  │  │
│                              │                    │  │
│                              │ 📎 Add references  │  │
│                              └────────────────────┘  │
│                                                      │
│                     [Cmd + Enter to send]  [▶ Start] │
└─────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────┐
│ ← Gallery │ Conversation Title              2 turns │
├─────────────────────────────────────────────────────┤
│ [img1] [img2] [img3] [img4] ...     (thumbnail strip)│
├─────────────────────────────────────────────────────┤
│                                                      │
│ ● EDIT 1                                             │
│   Replace the sea lion with a corgi                  │
│   ┌─────┐ ┌─────┐                                   │
│   │     │ │ ○   │ ← loading skeleton                │
│   │img1 │ │Gen..│                                   │
│   └─────┘ └─────┘                                   │
...
3. **E2E Test Flow**:
   ...
   - Click Edit on a thumbnail → modal opens
   - Enter edit prompt, select models, click Start
   - Redirected to conversation page
   - See turn 1 with loading skeletons → images appear progressively
   - Send another edit → turn 2 appears with progressive loading
